package com.example.realmdummyapp;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    private void ShowUpdateDialog(final DataModel dataModel,long id) {
        AlertDialog.Builder al=new AlertDialog.Builder(MainActivity.this);
        View view=getLayoutInflater().inflate(R.layout.data_input_dialog,null);
        al.setView(view);
            ...
            public void onClick(View v) {
                alertDialog.dismiss();
                        ...
                        backgroundRealm.executeTransactionAsync(new Realm.Transaction() {
                            @Override
                            public void execute(Realm realm) {
                                final DataModel dataModel1=realm.where(DataModel.class).equalTo("id",id).findFirst();
                                dataModel1.setAge(Integer.parseInt(age.getText().toString()));
                                dataModel1.setName(name.getText().toString());
                                dataModel1.setGender(gender.getSelectedItem().toString());
                                realm.copyToRealmOrUpdate(dataModel1);
                            }
                        });
